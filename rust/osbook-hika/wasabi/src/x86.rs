...
const ATTR_CAHE_DISABLE: u64 = 1 << 4;
...
pub unsafe fn write_es(selector: u16) {
    asm!(
        "mov es, ax", in("ax") selector
    )
}

/// # Safety
/// Anything can happen if the given CS is invalid.
pub unsafe fn write_cs(cs: u16) {
    // the MOV instruction CANNOT be used to load the CS register
    // Use far-jump(ljump) instead
    asm!(
...
impl fmt::Debug for InterruptInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            "
                {{
                    rip: {rip:#018X}, CS: {cs:06X},
                    rsp: {rsp:#018X}, SS: {ss:06X},
                    rbp: {rbp:#018X},

                    rflags:    {rflags:#018X},
                    error_code: {error_code:#018X},

                    rax: {rax:#018X}, rcx: {rcx:#018X},
                    rdx: {rdx:#018X}, rbx: {rbx:#018X},
                    rsi: {rsi:#018X}, rdi: {rdi:#018X},
                    r8:  {r8:#018X},  r9:  {r9:#018X},
                    r10: {r10:#018X}, r11: {r11:#018X},
                    r12: {r12:#018X}, r13: {r13:#018X},
                    r14: {r14:#018X}, r15: {r15:#018X},
                }}
            ",
            rip = self.ctx.rip,
            cs = self.ctx.cs,
