use crate::{
    acpi::AcpiMcfgDescriptor,
    error, info,
    result::Result,
    x86::{with_current_page_table, PageAttr},
    xhci::PciXhciDriver,
};
use core::{
    fmt,
...
impl Pci {
    ...
    pub fn try_bar0_meme64(&self, bdf: BusDeviceFunction) -> Result<BarMem64> {
    ...
    pub fn enable_bus_master(&self, bdf: BusDeviceFunction) -> Result<()> {
        self.set_command_and_status_flags(bdf, 1 << 2 /* Bus Mster Enable */)
    }
    pub fn disable_interrupt(&self, bdf: BusDeviceFunction) -> Result<()> {
        self.set_command_and_status_flags(bdf, 1 << 10 /* Interrurpt Disable */)
    }
